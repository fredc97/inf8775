...
void evolvePopulation(Population &population, Game &game)
{
 ...
 //           cout<<"Survivors - "<<pop.individuals.size()<<"\n";

            //cout<<pop.individuals.size()<<"\n";
        //}
        
        pop = selectBestOnes(pop, game.n_packs, game);
        cout << pop.individuals.at(0).packs << "\n";
        cout << fitnessIndividual(pop.individuals.at(0), game) << "\n";
        count++;
        //cout<<"On recommence - "<<pop.individuals.size()<<"\n";
       /*        cout << fitnessIndividual(pop.individuals.at(2), game) << "\n";
                cout << fitnessIndividual(pop.individuals.at(8), game) << "\n";
        cout << fitnessIndividual(pop.individuals.at(10), game) << "\n";
        cout << fitnessIndividual(pop.individuals.at(12), game) << "\n";
        cout << fitnessIndividual(pop.individuals.at(15), game) << "\n";
       */
        

       // mutationIndividual(pop.individuals.at(0),game);
...
Population mutation(Population &pop, Game &game) {
    ...
    while (mutations < MUTATIONS) {
        int ind = rand() % popTemp.individuals.size();
            ...
            /*for (int i = 0; i < pop.individuals.at(ind).packs.size(); i++ ) {
                cout<<"AVANT - PACK "<<i<<" : "<<scorePack(pop.individuals.at(ind).packs.at(i), game)<<"\n";
            }*/
            ...
            mutations++; 
            /*for (int i = 0; i < pop.individuals.at(ind).packs.size(); i++ ) {
                cout<<"APRÈS - PACK "<<i<<" : "<<scorePack(pop.individuals.at(ind).packs.at(i),game)<<"\n";
            }*/
   ...
   int pack_1, pack_2;
    do {
     ...
     pack_2 = rand() % ind.packs.size();
        ...
        pack1.totalValue = scorePack(pack1, game);
        pack2.totalValue = scorePack(pack2, game); 
...
Individual crossoverRandom(Individual &individual1, Game &game)
{
    ...
    for (int i = 0; i < genesPreserved; i++)
    {
        ...
        // Make take the wrong pack, because life is like this
...
void selection(Population &population, Game &game, Individual &winner_1, Individual &winner_2)
{
    ...
    //cout<<"VAINQUEUR FINAL 1 -------------> "<<winner_index_1<<"\n";
    //cout<<"VAINQUEUR FINAL 2 -------------> "<<winner_index_2<<"\n";

    //int test = fitnessIndividual(winner_1, game);
    //int test2 = fitnessIndividual(winner_2, game);
    //cout<<test<<" and "<<test2<<"\n";
    //    winners.winner1 = winner_1;
    //  winners.winner2 = winner_2;
    // return winners;
}
